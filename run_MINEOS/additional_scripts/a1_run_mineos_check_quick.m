...
% clear
tic
str_timeout_sec = '20'; % number of seconds after which to timeout
...
%     com = ['cat run_nohang.s | mineos_nohang_old > ',LOG];
    com = ['cat run_nohang.s | gtimeout ',str_timeout_sec,' mineos_nohang > ',LOG];
    [status,log] = system(com);
% % %     fprintf(fid,log);
    iflag_fail = 0;
    if status ~= 0     

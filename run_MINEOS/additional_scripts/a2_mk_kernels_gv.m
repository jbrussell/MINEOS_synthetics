...
%% run "frechet_gv" to generate the gv kernels
% Convert frechet to ascii
% Make GV Frechet Kernels
disp('--- Make GV Frechet Kernels ---');
...
[status,log] = system(com);
if status ~= 0     
    error( 'something is wrong at frechet_gv')
end
...
vp = VPV/1000;
r = 6371-R/1000;
%% Convert GV Frechet kernels to ascii with phase-velocity sensitivity
% Will do this for all periods of interest

disp('--- Convert Frechet GV to ascii ---');
...
save2pdf([FRECHETPATH,'CARD_Vs_kernels_gv_',lower(TYPE),'_',CARDID,'_b',num2str(branch),'.',num2str(N_modes),'_',num2str(periods(1)),'_',num2str(periods(end)),'s.pdf'],fig1,1000)

%    savefile = [CARD,'_fgv.mat'];
